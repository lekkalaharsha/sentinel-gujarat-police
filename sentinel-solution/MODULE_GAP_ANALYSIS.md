...
1. **Multi-vendor claim risk.** We can honestly say the *architecture* is vendor-neutral (RTSP/ONVIF-based, no vendor-specific code baked into the core pipeline), but we cannot claim it's been *proven* against a second real vendor system, because the sandbox only exposes one. Need to decide how strongly to word this in the Solution Presentation — "designed for" vs. "demonstrated with."
2. **Private/commercial CCTV support.** The background text explicitly asks for "viewing capabilities for public-facing CCTV cameras installed by societies, malls, commercial establishments... wherever feasible and permitted." Nothing in the current build addresses this at all — not even a stub or a documented approach. Worth deciding whether this needs even a paragraph in the HLD (e.g., "same onboarding flow, contingent on the entity granting access") or whether it's fine to leave unaddressed given time constraints.
3. **External DB integration honesty.** VAHAN/SARTHI/eGujCop/AFIS/NAFIS are named as required integrations for "automated real-time alerts." We have zero code or stub for any of them — our watchlist is a local table we seed ourselves, not a live query against a real government database. This is already somewhat addressed in `STRATEGY.md`'s "query, don't copy" principle, but worth confirming the HLD is explicit that this is integration-ready *design*, not a working integration.
